...
- [x] Task 1.1: `git mv src/spike src/shared` — 8 files move; intra-package
      relative imports need no edits — `src/shared/`
- [x] Task 1.2: `git rm run_spike.py` (legacy Phase-0 entrypoint, superseded
      by `run_curation.py`) — `run_spike.py`
- [x] Task 1.3: `git rm src/shared/pipeline.py` (zero callers after 1.2; see
      contract §4.1 for the justification the auditor will check) —
      `src/shared/pipeline.py`
- [x] Task 1.4: Replace `src/shared/__init__.py`'s docstring with the pinned
      text in contract §5 (describes the package's real cross-plane +
      Plane-B role) — `src/shared/__init__.py`
- [x] Task 1.5: Replace `src/shared/config.py`'s docstring with the pinned
...
- [x] Task 1.6: Rename the 4 env keys in `src/shared/config.py`:
...
- [x] Task 1.7: Reword the comment at `src/shared/config.py:47` ("keeps the
      spike cheap and fast" → "keeps each run cheap and fast") —
      `src/shared/config.py`
- [x] Task 1.8: Reword `src/shared/retrieval.py:3` ("fine for the spike's
      small corpus" → "fine for the current small corpus") —
      `src/shared/retrieval.py`

## Phase 2: Repoint production importers

- [x] Task 2.1: Repoint `from spike.feeds import RawItem` →
      `from shared.feeds import RawItem` — `src/curation/composite.py`
- [x] Task 2.2: Repoint 3 imports, rename the alias to `shared_config`, update
      the `spike_config.AWS_REGION` use at :31, and fix the
      `spike.bedrock.bedrock_client()` docstring at :5 —
      `src/curation/dynamo.py`
- [x] Task 2.3: Repoint 2 imports — `src/curation/interfaces.py`
- [x] Task 2.4: Repoint 3 imports; update the 3 docstrings at :3 (points at
...
- [x] Task 2.5: Repoint 2 imports + the module docstring at :5 —
      `src/curation/nodes.py`
- [x] Task 2.6: Repoint 2 imports + the comment at :11 —
      `src/curation/state.py`
- [x] Task 2.7: Repoint the import, rename the alias to `shared_config`, and
...
- [x] Task 2.8: Repoint 1 import + the docstring at :4 —
      `src/curation/tavily.py`
- [x] Task 2.9: Reword the module docstring ("Phase 0 spike curation loop" →
      "Phase 0 curation loop") — `src/curation/__init__.py`
- [x] Task 2.10: Update the 2 comment lines at :89-90 (`spike/config.py` →
      `shared/config.py`) — `src/curation/config.py`
- [x] Task 2.11: Repoint `from spike import config` at :42 —
      `runtime_app.py`
- [x] Task 2.12: Repoint 2 imports at :31-32 — `run_curation.py`
- [x] Task 2.13: Repoint 2 imports at :18-19; update the module docstring
...
- [x] Task 3.1: Repoint 2 imports (:26, :97) + 4 docstring lines (:5 — now
      references `run_curation.py`, :6, :12, :86, :88) — `tests/conftest.py`
- [x] Task 3.2: Repoint 2 imports (:23-24) + 5 docstring lines
      (:1, :5, :10, :14, :15) — `tests/test_bedrock_usage.py`
- [x] Task 3.3: Repoint 1 import (:26) — `tests/test_dynamo_store.py`
- [x] Task 3.4: Repoint 3 imports (:32, :143, :300) + 2 comments (:199, :228,
...
- [x] Task 3.5: Repoint 2 imports (:15, :173) + 4 comments (:103, :125, :144,
      :170). **Do NOT rename
      `test_upsert_writes_seen_sorted_and_cards_batch_matching_spike_save_shape`
      at :104** — `tests/test_local_store.py`
- [x] Task 3.6: Repoint the 2 `summary_module.spike_config` attribute paths at
...
- [x] Task 3.7: Update the comment at :62 (`spike.feeds._clean` →
      `shared.feeds._clean`) — `tests/test_tavily.py`
- [x] Task 3.8: Rename the 3 keys (:21-23) and rewrite the header line at :1
      ("AI Radar — Phase 0 spike configuration.") — `.env.example`
- [x] Task 3.9: `.spike_cache/` → `.ai_radar_cache/` — `.gitignore`
- [x] Task 3.10: `.spike_cache/` → `.ai_radar_cache/` — `.dockerignore`
- [x] Task 3.11: Update the `description` field (drop "(Phase 0 spike)").
      **Do not touch dependencies, groups, or `[tool.uv]`; do not run
      `uv add`/`uv sync`; `uv.lock` must show no diff** — `pyproject.toml`
- [x] Task 3.12: Update `README.md`:
      ...
      — `README.md`
- [x] Task 3.13: Update `CLAUDE.md`: the layout block (:63 `run_spike.py`
...
- [x] Task 3.14: `src/spike/` → `src/shared/` at :16 —
      `.claude/agents/sdd-architect.md`
- [x] Task 3.15: Tick the FU1 checkbox `- [ ]` → `- [x]`. **One character.
      Change nothing else in `specs/` or `docs/`** —
      `specs/run-observability/tasks.md`

## Phase 4: Validate

- [x] Task 4.1: Sweep stale bytecode so it cannot mask a missed import:
      `find . -name __pycache__ -not -path "./.venv/*" -prune -exec rm -rf {} +`
- [x] Task 4.2: Run gate **G1** (code+config hard zero) and paste the output
      (must be empty) into `audit.md` —
      `grep -rin "spike" src/ infra/ *.py pyproject.toml Dockerfile .dockerignore .gitignore .env.example`
- [x] Task 4.3: Run gates **G2** (tests: exactly the 3 enumerated `def test_…`
...
- [x] Task 4.8: Verify history: `git log --follow src/shared/config.py | head`
      shows pre-rename commits, and `git status` reports renames (`R`) rather
      than add/delete pairs for the 7 moved files
- [x] Task 4.9: Confirm `git diff uv.lock` is empty and `git diff
      pyproject.toml` touches only the `description` line
- [x] Task 4.10: **Scope-creep review** — walk the full diff and confirm every
