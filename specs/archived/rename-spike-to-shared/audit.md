...
- [x] **Human confirms Tasks H1-H3 done** and `seen.json` still has its entries —
      **DONE 2026-09-17.** Human live-verified all three directly in the repo:
      `.spike_cache/` no longer exists on disk; `.ai_radar_cache/` exists in its
      place (H1). `.env`'s keys are already `AI_RADAR_MAX_ITEMS`/
      `AI_RADAR_PER_FEED`/`AI_RADAR_TOP_K`, no stale `SPIKE_*` keys remain (H2).
      Ran the exact contract.md §12 verification snippet
      (`uv run python -c "..."` reading `shared.config`): output was
      `CACHE_DIR=.ai_radar_cache MAX_ITEMS=8 PER_FEED=5 TOP_K=4` and
      `seen entries: 32` — non-zero, proving H1 did not orphan the dedup cache
      (H3). This was the one open item blocking full close-out; it is now
      resolved.
- [ ] FU-A and FU-B carried forward into the next spec's backlog (unchanged from
      this spec — no new follow-ups added). **Not a blocker to archiving**: both
      remain open follow-ups by design (contract explicitly defers them, see
      `tasks.md` "Follow-ups / Not This Spec"), not items this spec promised to
      resolve.
